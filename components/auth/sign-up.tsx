                ...
                <div className="w-full">
                    <div className="rounded-3xl bg-white p-8 shadow-sm backdrop-blur sm:p-10">
                        ...
                        <form action={formAction} className="space-y-5" noValidate>
                            ...
                            {/* Name */}
                            <div>
                                <FormGroup id="name" name="name" type="text" required={true} label="Name" disabled={isPending} isError={formState?.errors?.name?.length > 0} errorMessage={formState?.errors?.name?.length ? formState.errors.name[0] : ""} />
                            </div>
                            {/* Email */}
                            <div>
                                <FormGroup id="email" name="email" type="email" required={true} label="Email" disabled={isPending} isError={formState?.errors?.email?.length > 0} errorMessage={formState?.errors?.email?.length ? formState.errors.email[0] : ""} />
                            </div>
                            {/* Password */}
                            <div>
                                <FormGroup id="password" name="password" type="password" required={true} label="Password" disabled={isPending} isError={formState?.errors?.password?.length > 0} errorMessage={formState?.errors?.password?.length ? formState.errors.password[0] : ""}>
                            ...
                            <div className="flex items-start gap-3">
                                <input id="termsAccepted" name="termsAccepted" type="checkbox" className="mt-1 h-4 w-4 rounded border-zinc-300 text-zinc-900 focus:ring-zinc-900/20" disabled={isPending}
                                />
